  ...
  useEffect(() => {
    const fetchPrompt = async () => {
      try {
        ...
        const response = await fetch('http://localhost:3000/writing/random', {
          method: 'GET',
          headers: {
  ...
  const handleSubmit = async () => {
    if (answer.trim().length > 0) {
      try {
        ...
        // Step 1: Analyze user writing
        const analyzeResponse = await fetch('http://localhost:3000/writing/analyze', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
        ...
        const cumulativeScore = analyzeData.cumulativeScore;

        // Step 2: Submit writing score
        const submitResponse = await fetch('http://localhost:3000/writing/submit', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ promptId: prompt?.prompt_id, score: cumulativeScore }),
        });
        ...
        setEvaluationResult({ cumulativeScore });
        setShowModal(true);
      ...
      <div className="max-w-4xl w-full space-y-8 z-10">
        ...
        >
          <div className="p-8">
            ...
            >
              <h2 className="text-xl font-semibold text-indigo-800 mb-2">Writing Prompt</h2>
              <p className="text-gray-700">{prompt?.prompt || 'Loading prompt...'}</p>
            </motion.div>
            ...
            <textarea
              ...
              className="w-full min-h-[300px] p-4 border-2 border-dashed border-purple-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 bg-blue-50 hover:shadow-lg placeholder-purple-300"
            ...
            <div className="flex justify-between items-center mt-6">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={handleQuit}
                className="text-purple-600 hover:text-purple-800 flex items-center bg-purple-100 rounded-lg px-4 py-2 hover:bg-purple-200 transition"
              >
              ...
              <motion.button
                ...
                className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition duration-300 flex items-center shadow-md group"
              >
          ...
          {showModal && (
            <Modal isOpen={showModal} onClose={handleCloseModal}>
              <motion.div
                ...
                className="bg-gradient-to-br from-indigo-300 to-purple-400 rounded-xl shadow-2xl p-8 max-w-md mx-auto text-center"
              >
                <div className="mb-6">
                ...
                <button
                  ...
                  className="w-full py-3 bg-white text-purple-600 rounded-lg hover:bg-gray-100 transition duration-300"
                >
                  Return to Dashboard
                </button>
              </motion.div>
